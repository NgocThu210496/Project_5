package ra.project_5.sercurity.jwt;
...
public class JwtProvider {
    ...
    public String generateAccessToken(UserDetails userDetails) {
        return Jwts.builder()
                .setSubject(userDetails.getUsername())//Nội dung sinh token. trong payload
                .setIssuedAt(new Date())//Ngày hiệu lực
                .setExpiration(new Date(System.currentTimeMillis() + JWT_ACCESS_TOKEN))
                .signWith(SignatureAlgorithm.HS512, JWT_SECRET) //ma hoa
                .compact();
    }
    ...
    public boolean validateToken(String token) {  //khi client vao thi no se cap 1 token
        try {
            Jwts.parser().setSigningKey(JWT_SECRET).parseClaimsJws(token);
            return true;

        } catch (ExpiredJwtException e) {
            log.error("Failed -> Expired Token Message {}", e.getMessage()); //token het han
        } catch (UnsupportedJwtException e) {
            log.error("Failed -> Unsupported Token Message {}", e.getMessage()); // k ho tro token nay
        } catch (MalformedJwtException e) {
            log.error("Failed -> Invalid Format Token Message {}", e.getMessage()); //loi chu ky
